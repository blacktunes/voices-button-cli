...
module.exports = async function create(name, targetDir) {
  ...
  await require('./ga/ga')(checkbox.includes('ga'), checkbox.includes('pwa'), targetDir)
  ...
  log(`请把语音文件放入${chalk.cyan('public/voices')}目录`, 'TIPS')
  log(`然后在项目根目录运行${chalk.cyan(`btn voices`)}生成${chalk.cyan('voices.json')}`, 'TIPS')
}
