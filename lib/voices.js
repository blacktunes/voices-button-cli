...
module.exports = async function create(cwd) {
  warn('该功能尚未完善，可能会有异常情况出现')
  let list = {
    category: [],
    voices: []
  }
  if (fs.existsSync(path.resolve(cwd, './src/setting/translate/voices.json'))) {
    list = fs.readJSONSync(path.resolve(cwd, './src/setting/translate/voices.json'))
  }
  ...
  if (fs.existsSync(path.resolve(cwd, './src/setting/translate/voices.json'))) {
    fs.renameSync(path.resolve(cwd, './src/setting/translate/voices.json'), path.resolve(cwd, `./src/setting/translate/voices_${Date.now()}.json`))
  }
  fs.writeJSONSync(path.resolve(cwd, './src/setting/translate/voices.json'), list, {
    spaces: 2
  })
